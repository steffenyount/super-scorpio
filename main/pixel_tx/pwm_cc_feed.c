...
volatile bool __scratch_y("super_scorpio") pwm_cc_feed_done = false;


// dma_pwm_cc_feed: pio1 RX FIFOs (tx_bytes_to_pwm_cc) -> pio1 TX FIFOs (loop back) + PWM slice CCs
void __not_in_flash_func(init_dma_pwm_cc_feed)() {
    dma_channel_claim(DMA_PWM_CC_FEED_CHAN);
    ...
    channel_config_set_ring(&dma_channel_ctrl, false, 4);
    ...
    dma_channel_set_config(DMA_PWM_CC_FEED_CHAN, &dma_channel_ctrl, false);

    dma_channel_set_read_addr(DMA_PWM_CC_FEED_CHAN, pio1->rxf, false);
//    dma_channel_set_write_addr(DMA_PWM_CC_FEED_CHAN, &pwm_hw->slice[slice_num].cc, false);
    dma_channel_set_trans_count(DMA_PWM_CC_FEED_CHAN, 1, false);
}
